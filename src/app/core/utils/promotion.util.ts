...
export function calculateCartSummary(items: CartItem[]): CartSummary {
  ...
  const subtotal = round(items.reduce((sum, item) => {
    const toppingsTotal = item.toppings.reduce((toppingSum, topping) => toppingSum + topping.price, 0);
    return sum + (item.drink.price + toppingsTotal) * item.quantity;
  }, 0));

  const thresholdDiscount = subtotal >= 12 ? subtotal * 0.25 : 0;
  const cheapestDrinkDiscount = drinkCount >= 3
    ? Math.min(...items.map((item) => item.drink.price + item.toppings.reduce(
